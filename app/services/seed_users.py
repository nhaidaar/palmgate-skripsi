...
def parse_seed_identity(label: str) -> tuple[str, str]:
    nim, separator, name = label.partition("_")
    if not separator or not nim.strip() or not name.strip():
    ...
    *,
    ...
    if person_dirs:
        for person_dir in person_dirs:
            try:
                nim, name = parse_seed_identity(person_dir.name)
                ...
                db.add_user(name, result.embedding, nim=nim, individual_embeddings=result.individual_embeddings)
                created.append(name)
    ...
    for path in _seed_image_paths(seed_dir):
        try:
            nim, name = parse_seed_identity(path.stem)
            if name in existing_names:
                skipped.append(name)
                continue
